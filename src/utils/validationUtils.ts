...
const validatePairOfGradesUsingRegex = async (value1: string, value2: string, regexp: RegExp) => {
    const validationResult =
        (value1.length === 0 || regexp.test(value1)) && (value2.length === 0 || regexp.test(value2));
...
export const validateGradeSystemAndGradesInput = async (
    gradingSystem: RouteGradingSystem,
    minDifficulty: string | undefined,
    maxDifficulty: string | undefined,
): Promise<{ minDifficulty: string; maxDifficulty: string }> => {
    ...
    switch (gradingSystem) {
        ...
        case RouteGradingSystem.UIAA:
            minDifficultyOrDefault =
                minDifficulty && minDifficulty.length > 0 ? minDifficulty : UIAA_SYSTEM_DEFAULT_MIN_GRADE;
            maxDifficultyOrDefault =
                maxDifficulty && maxDifficulty.length > 0 ? maxDifficulty : UIAA_SYSTEM_DEFAULT_MAX_GRADE;
